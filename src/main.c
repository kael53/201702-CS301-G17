	...
	for (int m = 256; m <= 2048; m *= 2) {
		...
		for (int n = 8; n <= 24; n += 8) {
			for (int exp = 1; exp <= 50; exp++) {
				...
				for (int i = 0; i < 2 * m; ++i)
					clauses[i] = (((rand() % n) + 1) //pick a non-zero literal at random
							* ((rand() % 2) ? 1 : -1)) //apply negation
							* (((i & 1) && (rand() % 2)) ? 0 : 1); //second literal in a clause can be zero(empty)
				...
				t0 = omp_get_wtime();
				filePrintScore(solution = solveWithHC(solver), f);
				t1 = omp_get_wtime();
				fprintf(f, "%f,", t1 - t0);
				...
				t0 = omp_get_wtime();
				filePrintScore(solution = solveWithSA(solver, 10*m*n, 100, 0.85, n*m), f);
				t1 = omp_get_wtime();
				fprintf(f, "%f,", t1 - t0);
				...
				t0 = omp_get_wtime();
				filePrintScore(solution = solve(solver), f);
				t1 = omp_get_wtime();
				fprintf(f, "%f\n", t1 - t0);
